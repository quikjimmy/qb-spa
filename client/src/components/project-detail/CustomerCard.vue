...
<template>
  ...
  >
    <!-- Row 1: Customer name + (desktop-only inline) integration icons · Status -->
    <div class="flex items-center justify-between gap-3 px-4 sm:px-5 pt-3 pb-1 sm:pb-3">
      <div class="flex items-center gap-2.5 min-w-0 flex-1">
        ...
        <!-- Integration icon strip — INLINE on desktop only, so the customer
             name doesn't get squeezed on mobile. Mobile renders this same
             strip on its own row beneath the name. -->
        <div class="hidden sm:flex items-center gap-1.5 ml-1 shrink-0 text-slate-400">
          <a
            v-if="qbHref"
            :href="qbHref"
            target="_blank"
    ...
    <!-- Mobile-only integration row (under the name so it doesn't crowd the H1) -->
    <div
      v-if="qbHref || driveHref || enerfloHref || arrivyHref"
      class="sm:hidden flex items-center gap-1.5 px-4 pb-2.5 text-slate-500"
    >
      <a
        v-if="qbHref"
        :href="qbHref"
