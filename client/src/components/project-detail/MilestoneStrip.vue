    ...
    >
      ...
      <div class="relative w-full flex items-center justify-center mt-1.5 mb-1 h-4">
        ...
        <!-- Inter-step transit pill — centered on the boundary between this
             ...
             (px-0.5, 9px text) so it stays between the dots at 360–430px
             widths where the cells are only ~38–46px wide. -->
        <span
          v-if="transitFor(s.id) && i < steps.length - 1"
          class="absolute right-0 translate-x-1/2 top-1/2 -translate-y-1/2 z-10 inline-flex items-center justify-center px-0.5 sm:px-1.5 h-[12px] sm:h-[15px] text-[9px] sm:text-[10.5px] font-semibold tabular-nums leading-none select-none whitespace-nowrap rounded-full ring-1 ring-inset"
          :class="[
            transitFor(s.id)!.tier === 'good' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : '',
            transitFor(s.id)!.tier === 'warn' ? 'bg-amber-50  text-amber-700  ring-amber-200'  : '',
            transitFor(s.id)!.tier === 'bad'  ? 'bg-rose-50   text-rose-700   ring-rose-200'   : '',
          ]"
